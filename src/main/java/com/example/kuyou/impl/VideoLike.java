...
@Repository
@ComponentScan
public class VideoLike implements VideoInfo {
    ...
    private static final String SELECT_VIDEO_MUSIC  =
            "select video.v_id, v_cover, v_content, v_creator, v_date, " +
                    ...
                    "from video, view, music, video_thumb " +
                    "where video_thumb.u_id = ? and video_thumb.v_id = video.v_id" +
                    " and view.view_id = video.view_id " +
                    "and music.m_id = video.m_id;";

    @Autowired
    public VideoLike(JdbcOperations jdbcOperations){
